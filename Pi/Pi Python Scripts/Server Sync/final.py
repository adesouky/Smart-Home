...
def main():
    ...
    while True:
        ...
        if changeOccurred() == "true": #Assuming changeOccured() returns string "1" if true
            ...
            #for demsonstation
            if buttonsStatus['ac1'] == "on": #Temperature : Living Room
                GPIO.output(5,GPIO.HIGH)
            else:
                GPIO.output(5,GPIO.LOW)
            
            if buttonsStatus['hum1'] == "on": #Lighting : Living Room
                GPIO.output(6,GPIO.HIGH)
            else:
                GPIO.output(6,GPIO.LOW)
            
            if buttonsStatus['hum2'] == "on": #Humidity :Living Room
                GPIO.output(13,GPIO.HIGH)
            else:
                GPIO.output(13,GPIO.LOW)
            
            if buttonsStatus['ac2'] == "on": #Front Door : Door 1 
                GPIO.output(19,GPIO.HIGH)
            else:
                GPIO.output(19,GPIO.LOW)
            if buttonsStatus['heat2'] == "on": #Humidity :Living Room
                GPIO.output(26,GPIO.HIGH)
            else:
                GPIO.output(26,GPIO.LOW)
            
            if buttonsStatus['lamp2'] == "on": #Front Door : Door 1 
                GPIO.output(20,GPIO.HIGH)
            else:
                GPIO.output(20,GPIO.LOW)
            if buttonsStatus['lamp1'] == "on": #Front Door : Door 1 
                GPIO.output(21,GPIO.HIGH)
            else:
                GPIO.output(21,GPIO.LOW)
            if buttonsStatus['heat1'] == "on": #Front Door : Door 1 
